...
// ---------------------------------------------------------------------------
// HL2SB: GMod (Garry's Mod 13, update 143) player activity set.
//
// HL2MP declared only 7 states per hold type (IDLE / RUN / IDLE_CROUCH /
// WALK_CROUCH / GESTURE_RANGE_ATTACK / GESTURE_RELOAD / JUMP). GMod's 9-way
// player system also uses WALK, SWIM, SWIM_IDLE and SIT, and it has many more
// aim layers ("hold types"): REVOLVER, DUEL, KNIFE, MELEE2, FIST, CAMERA,
// PASSIVE, MAGIC, SCARED, SUITCASE, ZOMBIE ... - see
// https://wiki.facepunch.com/gmod/Player_Animations
//
// Everything below is *appended*, never inserted: the numeric value of an
// activity is what studiomdl baked into a model's sequence table, so inserting
// anywhere above would silently shift every later HL2MP id and break the
// shipped models. Appending leaves all existing ids untouched.
//
// Names are taken from GMod's own activity string table (ACT_* strings in
// bin/win64/server.dll, 273 of them) and cross-checked against the sequences
// the GMod animation models actually declare (models/m_anm.mdl: 220).
// ---------------------------------------------------------------------------

	// --- generic (no hold type) ---
	ACT_HL2MP_WALK,
	ACT_HL2MP_SWIM,
	ACT_HL2MP_SWIM_IDLE,
	ACT_HL2MP_SIT,
	// GMod only - alternate run styles (sprint / charge / panic / protect)
	ACT_HL2MP_RUN_FAST,
	ACT_HL2MP_RUN_CHARGING,
	ACT_HL2MP_RUN_PANICKED,
	ACT_HL2MP_RUN_PROTECTED,

	// --- HL2MP hold types, GMod states they never had ---
	// PISTOL
	...
	// PHYSGUN (GMod's aim layer name for it is "gravgun", the activity
	// suffix stayed PHYSGUN)
	...
	// --- GMod aim layers (hold types) that HL2MP never had ---
	// REVOLVER (two handed small firearms - weapon_357)
	...
	// CAMERA (gmod_camera / physgun-like remote camera)
	...
	// PASSIVE (weapon held down / not aiming in front)
	...
	// ZOMBIE (z_anm.mdl) and its walk variants
